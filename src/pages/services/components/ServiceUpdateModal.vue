...
const emit = defineEmits<{
  ...
  (e: 'updated', data: Service): void
}>()

const formRef = ref()

const formState = reactive<Omit<Service, 'price'> & { price: string }>({
  id: 0,
  name: '',
  description: '',
  price: '',
...
watch(
  () => props.service,
  (val) => {
    if (val) Object.assign(formState, { ...val, price: String(val.price) })
  },
  { immediate: true },
)
...
function onSubmit() {
  formRef.value
    ?.validate()
    .then(() => {
      emit('updated', { ...formState, price: Number(formState.price) })
